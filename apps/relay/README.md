...
- Optional SSE wakeups for low-latency stream polling.
...
The authoritative API contract lives in `CONTRACT.md`, `OPENAPI.yaml`, and `TEST_VECTORS.md` at the repo root.
...
- Jobs and payload events are queued and retrieved by polling; optional SSE wakeups signal which streams to poll.
...
flowchart LR
  ...
  subgraph Relay[Relay Service]
    ...
    Auth[Auth Verifier]
    Store[SQLite + sqlc]
    StreamHub["Stream Hub (SSE)"]
    Ret[Retention Loop]
    Metrics[Metrics Server]
  end

  Bot <--> API
  Bot -. "SSE /v0/stream (wakeups)" .-> StreamHub
  API --> RL --> Auth --> Store
  API --> StreamHub
  Store <--> Ret
...
- `CONTRACT.md` (repo root)
- `OPENAPI.yaml` (repo root)
- `TEST_VECTORS.md` (repo root)

Contracted endpoints (see `OPENAPI.yaml` for request/response schemas):
...
Additional implementation endpoints (not yet in the contract; subject to change):
...
- `POST /v0/offers/{offer_id}/resume`

Jobs:
- `POST /v0/jobs/{job_id}/payment_sent`
- `POST /v0/jobs/{job_id}/charge/reissue`
